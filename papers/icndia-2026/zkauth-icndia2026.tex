...
\textit{Environment note:} Measurements are from the Node.js/snarkjs
pipeline. Browser WebAssembly timings will differ by engine and SIMD
support; browser-specific benchmarks and multi-device cross-platform
comparison are planned follow-on work.

\textit{LSTM note:} Table~\ref{tab:lstm_perf} entries marked \textbf{[TBD]}
are pending evaluation on an authentic user behavioral dataset; they do not
affect the ZKP performance claims reported here.
...
\caption{ZKP Authentication Performance ($n=120$, Apple M4, Node.js/snarkjs
v0.7.4. Server verify includes deliberate 50\,ms timing-attack mitigation
padding; see Section~\ref{sec:security}.)}
\label{tab:zkp_perf}
\centering
\begin{tabular}{lcccc}
\toprule
...
\midrule
\textbf{Total E2E}  & 119.33 & \textbf{132.51} & 143.06 & 27.40 \\
\midrule
Disclosure prove    & 167.07 & 172.97 & 180.87 & 24.71 \\
Disclosure verify$^\dagger$   & 5.31  & 5.86  & 7.72  & 0.72  \\
\bottomrule
\multicolumn{5}{l}{\footnotesize $^\dagger$Raw local snarkjs verification; no HTTP round trip or T14 timing pad.}\\
\end{tabular}
\end{table}
