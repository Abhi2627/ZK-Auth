...
and headless proving. Browser WASM measurements ($n=30$, Chrome, Web Worker)
are also reported; mobile WebView results are planned follow-on work.
...
\caption{ZKP Proof Generation Across Environments (Apple M4, snarkjs v0.7.4).
Server verify includes 50\,ms T14 pad~\S\ref{sec:security}.
$^\dagger$Raw local snarkjs; no HTTP/pad. $^\ddagger$Chrome Web Worker internal only.}
\label{tab:zkp_perf}
\centering
\begin{tabular}{lcccc}
\toprule
...
and scroll telemetry alongside mouse dynamics; multi-device cross-platform
benchmark collection for proof generation across heterogeneous hardware;
browser WASM proof generation timing; circuit optimization for reduced
